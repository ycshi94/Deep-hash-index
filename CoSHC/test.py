...
def validate(model, pool_size, sim_measure):
    ...
    code_hashs = load_h5('./data/github/code_test_hash.h5')
    desc_hashs = load_h5('./data/github/desc_test_hash.h5')
    code_class = []
    ...
    for i in tqdm(range(len(code_reprs))):
        ...
        with torch.no_grad():
            code_batch_class = int(
                torch.max(F.softmax(class_model.getclass(code_batch_v), dim=1), dim=1)[1].data.numpy().squeeze())
