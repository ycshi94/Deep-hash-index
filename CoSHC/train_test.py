...
def train(args):
    ...
    train_set = CodeSearchDataset(data_path,config['code_vec'], config['desc_vec'])
    valid_set = CodeSearchDataset(data_path,config['code_vec_valid'], config['desc_vec_valid'])

    data_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=config['batch_size'], shuffle=True,
...
def validate(valid_set, model, pool_size, K, sim_measure):
    ...
    for k in tqdm(range(0, n_processed, pool_size)):
        ...
        for i in range(min(len(code_pool), pool_size)):
